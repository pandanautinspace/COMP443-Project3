...
@main def main(): Unit = {
        ...
        val rankedPages: List[RankedWebPage] = PageRank.pagerank(pages).map((s, d) => RankedWebPage(pages(s),d)).toList // call PageRank.???? here
        ...
        while {
          ...
          val searchedPages: List[SearchedWebPage] = PageSearch.tfidf(rankedPages, terms).zipWithIndex.map((c, i) => SearchedWebPage(rankedPages(i), c)) // call PageSearch.???? here
